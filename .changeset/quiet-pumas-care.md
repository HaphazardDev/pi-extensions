---
"@haphazarddev/pi-ask-user-question": minor
"@haphazarddev/pi-copy-code-block": minor
"@haphazarddev/pi-interactive-code-review": minor
"@haphazarddev/pi-vim-quit": minor
---

Update these extensions to the new `@earendil-works/*` pi package scope.

Consumers should install the new `@earendil-works` pi packages to satisfy peer dependencies. `@haphazarddev/pi-copy-code-block` also switches its direct clipboard dependency from `@mariozechner/clipboard` to `clipboardy`.
