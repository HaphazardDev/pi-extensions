...
**Architecture:** A session-scoped `BackgroundJobManager` owns child processes and disk-backed logs. A thin Pi adapter registers one shell-semantic start tool plus read-only lifecycle tools, updates a footer status, injects completion messages, and exposes `/background-bash` as a custom two-level browser (job list → output detail). Ordinary Pi `bash` remains unchanged; all jobs are session-owned and cleaned up during `session_shutdown`.

**Tech Stack:** TypeScript 7, Node.js `child_process`/`fs`, Pi Coding Agent 0.80.10 extension API, Pi TUI, TypeBox, Vitest.
...
- Completion messages wake the agent as a follow-up only when requested by the start call; UI notifications and status always update.
- The status surface reports running and total jobs, e.g. `⚙ 2 running • 4 jobs • /background-bash`.
- `/background-bash` opens a keyboard-navigable list. Enter opens output; up/down or j/k navigate; r refreshes; s stops a running job; q/Escape returns or closes.
...
3. Verify RED.
4. Implement the list/detail component and `/background-bash` command loop.
5. Update `ctx.ui.setStatus` after every lifecycle event.
...
2. Document `/background-bash`, tool contracts, keybindings, process ownership, log retention, and platform limits.
